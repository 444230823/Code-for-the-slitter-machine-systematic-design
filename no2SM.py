...
def judgeNum():                       #数字判断
    ...
    numpredict = int(lastnum + s*(deltaL//api.deltaP)) if (deltaL%api.deltaP < (api.deltaP/2)) else int(lastnum + s*(deltaL//api.deltaP + 1)) #预测这个标应为多少
...
def dataDeal(opendata):               #x42有数字再调用
    ...
    TrainSample['CharLen'] = ' '
    result = divmod(api.lengthAct-deltaLen,5)     #获取米长的商和余数 判断条件1
...
def makeDecision(flawFileName):       #生成决策表
    ...
    badMetersFilepath = readPath + '\\' + flawFileName
    global badMetersdata
    badMetersdata = pd.read_csv(badMetersFilepath,encoding='GBK')
    api.badMeters = badMetersdata['Y坐标'].values.tolist()
    api.badMetersX = badMetersdata['X坐标'].values.tolist()
    api.badPics = badMetersdata['画像'].values.tolist()
    api.zhengfan = badMetersdata['位置'].values.tolist()
    api.zhengfan.insert(0,0)
    api.badMetersDone = []
    for i in range(len(api.badMeters)+1):
        api.badMetersDone.append(0)
    ...
    api.jiedaiNow = (api.lengthAct - api.lengthDelta - api.LJiedai)if (api.lengthDelta<=0) else (api.lengthAct - api.lengthDelta + api.LJiedai)
    ...
    # for i in range(len(api.badMeters)-1):
    #     deltaX = api.badMeters[i+1]-api.badMeters[i]    # 下一个缺陷到当前缺陷的距离
    #     # 当距离不足以升到低速再降到0时
    #     if deltaX < (xLUp + xLDown):
    #         v = sqrt((api.badMeters[i+1]-api.badMeters[i]) / (1/2/api.aUp+1/2/api.aDown))
    #         pos.append(api.badMeters[i])
    #         act.append(1)   # 升速决策
    #         tar.append(v)

    #         x = api.badMeters[i] + v**2/(2*api.aUp)
    #         pos.append(x)
    #         act.append(0)   # 降速决策
    #         tar.append(0)

    #     # 当距离足够升到低速再降到0但是不足10m时，只上升到低速
    #     elif (deltaX < 10) & (deltaX >= (xLUp + xLDown)):
    #         pos.append(api.badMeters[i])
    #         act.append(1)   # 升速决策
    #         tar.append(vLow)

    #         x = api.badMeters[i+1] - xLDown
    #         pos.append(x)
    #         act.append(0)   # 降速决策
    #         tar.append(0)
    #     # 当距离超过10m时
    #     elif deltaX >= 10:
    #         nearestP = api.badMeters[i+1]//5*5
    #         pos.append(api.badMeters[i])
    #         act.append(1)   # 升速决策
    #         # 缺陷位置到最近的检测点距离不够从低速降到0
    #         if (api.badMeters[i+1]-nearestP) <= xLDown:
    #             checkP = nearestP-5     # 提前一个点进行检测
    #             # 上一个缺陷点到检测点的距离不够升到高速再降到低速
    #             if (checkP-api.badMeters[i]) <= (xHUp+xHDown):
    #                 tar.append(vLow)

    #                 pos.append(api.badMeters[i+1]-xLDown)
    #                 act.append(0)   # 降速决策
    #                 tar.append(0)
    #             # 先升到高速再降到低速再停止
    #             else:
...
class photoThread (threading.Thread):      #采集图像类
    ...
    def run(self):
        ...
        with open(path, 'a+', newline='') as f:
            titles = ['Index', 'StartLine', '1', '2', '3', '4','X11','X14','X42','Y11','Y12','Y21','Y22','Y31','Y32','Y41','Y42','Judgement','Speed','LengthSum','jogSpeed','Time']
            total = 0
                ...
                for j in range(50):
                    if api.modetype == 0:       #面积检测
                        ...
                        try:
                            ...
                            xLable = 0         
                        api.pointscount = int(float(api.numNow))
                        try:
                        ...
                        if (arealast == 1)&(areanow == 0):                      #出现面积符合的下降沿
                            if lastx == 0:                                      #第一个标，计数+1，更新上一个标的米数
                                api.pointscount += 1
                                print('point',api.pointscount)
                                lastx = api.lengthAct
                                api.LCamera = api.deltaP*int(float(api.numNow)) + (api.LView/2 - api.LView*float(xLable)/api.pixX) + api.timeDelay*api.SpeedLmitHigh/60
                                api.lengthDelta = api.lengthAct - api.LCamera
                            else:
                                deltax = api.lengthAct - lastx                  #与上一个标的距离
                                if deltax >= (api.deltaP*2/3):                  #距离大于标志间距的2/3才可能是正确的标
                                    ...
                                    if abs(e) <= 0.4*api.deltaP:                             #两个标的距离与附近的5的倍数之间的差值(用来判断当前面积是阴影还是真的标)
                                        if deltax%api.deltaP >= (api.deltaP/2):
                                            api.pointscount += (deltax//api.deltaP+1)    #中间漏掉的点可以补回来
                                            api.LCamera = api.deltaP*int(float(api.numNow)) + (api.LView/2 - api.LView*float(xLable)/api.pixX) + api.timeDelay*api.SpeedLmitHigh/60
                                            api.lengthDelta = api.lengthAct - api.LCamera
                                            print('point',api.pointscount)
                                        else:
                                            api.pointscount += (deltax//5)
                                            api.LCamera = api.deltaP*int(float(api.numNow)) + (api.LView/2 - api.LView*float(xLable)/api.pixX) + api.timeDelay*api.SpeedLmitHigh/60
                                            api.lengthDelta = api.lengthAct - api.LCamera
                                            print('point',api.pointscount)
                                        lastx = api.lengthAct
                        arealast = areanow
                        api.numNow = str(api.pointscount).zfill(4)
                        ...
                        api.LCamera = api.lengthAct - api.lengthDelta
                        ...
                        if msagint[tt] == 'correct':
                            ...
                            api.LCamera = api.deltaP*int(api.numNow) + (api.LView/2 - api.LView*float(msagint[0])/api.pixX) + api.timeDelay*api.SpeedLmitHigh/60
                            api.lengthDelta = msagint[tt+2] - api.LCamera

                            renew = pd.DataFrame([{'lastNum':api.numNow, 'lengthDelta':api.lengthDelta}])
                            renew.to_csv(tempPath,mode='w',index=False,header=True)

                            # print('lcamera:',api.LCamera)
                            ...
                            nownum = int(nowstr.replace(' ',''))
                            deltaL = abs(msagint[tt+2]-sparelast[tt+2])  
...
class Api:                                 #仅用于设定全局变量
    def __init__(self):
        ...
        # self.didaoDingWeiRead = 0       # 底刀定位调整(读回)
        # self.Meters_StopRead = 0        # 点动停机米数(读回)
        # self.speedJogRead = 0           # 点动停机速度(读回)
        # self.fangjuanSpeedRatioRead = 0 # 放卷牵引速比(读回)
        # self.tongkuanUpRead = 0         # 筒宽上(读回)
        # self.tongkuanDownRead = 0       # 筒宽下(读回)
        # self.shoujuanFSetUpRead = 0     # 收卷张力设定上(读回)
        # self.shoujuanFSetDownRead = 0   # 收卷张力设定下(读回)
        # self.zhuiduUpRead = 0           # 锥度上(读回)
        # self.zhuiduDownRead = 0         # 锥度下(读回)
        # self.circlesUpRead = 0          # 滑差环数上(读回)
        # self.circlesDownRead = 0        # 滑差环数下(读回)
        # self.xintongSizeUpRead = 0      # 芯筒尺寸上(读回)
        # self.xintongSizeDownRead = 0    # 芯筒尺寸下(读回)
        ...
        #相机相关
        self.deltaP = 0.314      # 两个标志间的距离
        self.LSend = 0          # 发送给PLC的缺陷停机位置
        self.LCamera = 0        # 相机正下方所对应的米长
        self.LView = 0.2        # 相机视野范围，m
        self.LJiedai = 3.62209  # 相机正下方到接带处中间位置的距离，m
...
class myMainWindow(QtWidgets.QMainWindow,Ui_MainWindow):    #主界面的函数
    def __init__(self):
        ...
        self.displayData=[api.Meters_Stop,api.lengthSet,api.xintong_widthDown,api.fangjuanFSetDown,api.Mo_WidthDown,api.Mo_thickDown,api.tongkuanUp,api.circlesUp,\
            api.shoujuanFSetUp,api.zhuiduUp,api.shoujuanFSetDown,api.zhuiduDown,api.circlesDown,api.tongkuanDown,api.SpeedLmitHigh,api.Accelerate_time,api.Slow_time,api.Stop_time,\
                int(api.badPoint),api.speedJog,api.didaoDingWei,int(api.Amount_LeftFlaw),api.badMeters[api.badPoint],api.alarmR,api.jiedaijinchi,api.deltaP,api.Meters_NextFlaw]
    ...
    def mainTimer200(self):     # 主循环
        ...
        if self.mainState == 0:     #准备阶段
        ...
        elif self.mainState == 11:  #检查并发送一次当前缺陷位置
            print('state',self.mainState)
            if api.MXlist[1][11]:   #瑕疵停机已经完成
            ...
            self.qianOrHou = int(api.jiedaiNow <= api.badMeters[api.badPoint])
            print('state 11 point:',api.badPoint,'qian or hou',self.qianOrHou)
            self.numPush = api.numNow
            self.mainState = 0
        # elif self.mainState == 11:  #检查并发送一次当前缺陷位置
        #     api.jiedaiNow = (api.lengthAct - api.lengthDelta - api.LJiedai)if (api.lengthDelta<=0) else (api.lengthAct - api.lengthDelta + api.LJiedai)
        #     for i in range(len(api.badMeters)-1):
        ...
        # elif self.mainState == 12:  #检查当前缺陷是否已处理
        #     if not api.badMetersDone[api.badPoint]:
        #         self.mainState = 13 if api.MXlist[1][11] else 14
        #     else:
        #         self.mainState = 14
        
        # elif self.mainState == 13:  #当前缺陷已处理
        #     if self.qianOrHou:      #如果停早了，下一个缺陷位置是badPoint加1
        #         api.badPoint += 1
        #     api.badMetersDone[api.badPoint-1] = 1
        #     print('point',api.badPoint-1,'done')
        #     if api.badPoint >= len(api.badMeters):
        #         api.badPoint = len(api.badMeters)-1
        #     api.Meters_NextFlaw = api.badMeters[api.badPoint] + api.lengthDelta + api.LJiedai       #缺陷位置米数,由相机坐标换算过来，加上相机到接带处的距离
        #     api.Amount_LeftFlaw = len(api.badMeters)-api.badPoint   #剩余缺陷数量
        #     client.write_registers (values=inverse(api.badPoint) + inverse(api.Amount_LeftFlaw) + inverse(api.Meters_NextFlaw), unit=2, address=188, data_format='!f')
        #     client.write_coils(50,[api.zhengfan[api.badPoint]])
        #     self.mainState = 14
        
        # elif self.mainState == 14:  #等待处理完当前缺陷后的下一个标码修正米长
        #     if self.numPush != api.numNow:  #读到了与按下缺陷检测按键时不一样的标
        #         api.Meters_NextFlaw = api.badMeters[api.badPoint] + api.lengthDelta + api.LJiedai       
        #         api.Amount_LeftFlaw = len(api.badMeters)-api.badPoint   #剩余缺陷数量
        #         client.write_registers (values=inverse(api.badPoint) + inverse(api.Amount_LeftFlaw) + inverse(api.Meters_NextFlaw), unit=2, address=188, data_format='!f')
        #         self.mainState = 0
        #     else:
        #         try:
        #             distance0 = abs(api.badMeters[api.badPoint] - api.badMeters[api.badPoint-1])
        #         except Exception:
        #             distance0 = api.badMeters[api.badPoint]
        #         if (distance0 <= api.deltaP) | (api.badPoint == len(api.badMeters)-1):
        #             self.mainState = 0
    ...
    def dataShow(self):         # 显示数据
        ...
        # #MX寄存器
        # request1 = client.read_holding_registers(0,10).registers
        # api.MXlist[0] = dec_to_binlist(request1[0],16)
        # api.MXlist[1] = dec_to_binlist(request1[1],16)
        # api.MXlist[2] = dec_to_binlist(request1[2],16)
        # api.MXlist[3] = dec_to_binlist(request1[3],16)
        # api.MXlist[4] = dec_to_binlist(request1[4],16)
        # api.MXlist[5] = dec_to_binlist(request1[9],16)
        ...
        self.spinBox_20.setRange(0,10000)
        ...
        # self.spinBox_28.setRange(0,10000)
        # self.spinBox.setValue(api.Meters_Stop)
        # # self.spinBox_2.setValue(api.SpeedLmitHigh)
        # self.spinBox_3.setValue(api.lengthSet)
        # self.spinBox_4.setValue(api.xintong_widthDown)
        # self.spinBox_5.setValue(api.fangjuanFSetDown)
        # self.spinBox_6.setValue(api.Mo_WidthDown)
        # self.spinBox_7.setValue(api.Mo_thickDown)
        # self.spinBox_8.setValue(api.tongkuanUp)
        # self.spinBox_9.setValue(api.circlesUp)
        # self.spinBox_10.setValue(api.shoujuanFSetUp)
        # self.spinBox_11.setValue(api.zhuiduUp)
        # self.spinBox_12.setValue(api.shoujuanFSetDown)
        # self.spinBox_13.setValue(api.zhuiduDown)
        # self.spinBox_14.setValue(api.circlesDown)
        # self.spinBox_15.setValue(api.tongkuanDown)
        # self.spinBox_16.setValue(api.SpeedLmitHigh)
        # self.spinBox_17.setValue(api.Accelerate_time)
        # self.spinBox_18.setValue(api.Slow_time)
        # self.spinBox_19.setValue(api.Stop_time)
        # self.spinBox_21.setValue(api.fangjuanSpeedRatio)
        # self.spinBox_22.setValue(api.speedJog)
        # self.spinBox_23.setValue(api.didaoDingWei)
        # self.spinBox_20.setValue(int(api.badPoint))
        # self.spinBox_24.setValue(int(api.Amount_LeftFlaw))
        # self.spinBox_25.setValue(api.badMeters[api.badPoint]-(api.SpeedLmitHigh/60)**2/(2*api.aDown))
        # self.spinBox_26.setValue((api.SpeedLmitHigh/60)**2/(2*api.aDown))
        # self.spinBox_27.setValue(api.Meters_NextFlaw)
        # self.spinBox_28.setValue(api.SpeedLmitHigh)
    ...
    def continueShow(self):
        ...
        self.lcdNumber_7.setPlainText(str(round(api.houshoujuanR,2)))
        self.lcdNumber_8.setPlainText(str(round(api.yagunBackUp,2)))
        self.lcdNumber_9.setPlainText(str(round(api.yagunBackDown,2)))
        self.lcdNumber_10.setPlainText(str(round(api.QianshoujuanR,2)))
        try:
        ...
        if api.zhengfan[api.badPoint]:
            self.textEdit.setPlainText('正面')
        else:
            self.textEdit.setPlainText('反面')
    ...
    def qtInput(self,a):
        ...
        api.badPoint            = int(self.spinBox_20.value())
        ...
        if self.sender() == self.spinBox_20:
            api.Meters_NextFlaw = api.badMeters[api.badPoint]       #缺陷位置米数
            api.Amount_LeftFlaw = len(api.badMeters)-api.badPoint   #剩余缺陷数量
            self.dataShow()
            if api.uiEA:
                client.write_registers (values=inverse(api.badPoint) + inverse(api.Amount_LeftFlaw) + inverse(api.Meters_NextFlaw), unit=2, address=188, data_format='!f')
    ...
    def show_pic(self):              # 显示缺陷图片
        imagePath0 = readPath + '\\img\\{0}.bmp'.format(api.badPics[api.badPoint])
        print('image path',imagePath0)
        os.startfile(imagePath0)
        # image0 = QtGui.QPixmap(imagePath0).scaled(581, 451)
        # self.graphicsView.setPixmap(image0)
    
    def openFlawFile(self):         # 打开缺陷文件
        flawFilePath = readPath + '\\ErrorData.csv'
...
class myKnifeAndPeifang(QtWidgets.QMainWindow,Ui_KnifeEdit):  #刀具操作界面
    ...
    def D1(self):
        ...
        for i in range(12):
            self.daoNum[i].setValue(i+1)
            self.doubleSpinBoxNum[i].setValue(daoFile.loc[i+1,'运行米长'])
        # self.doubleSpinBox.setValue(daoFile.loc[self.spinBox_2.value(),'运行米长'])
        # self.doubleSpinBox_2.setValue(daoFile.loc[self.spinBox_3.value(),'运行米长'])
        # self.doubleSpinBox_3.setValue(daoFile.loc[self.spinBox_4.value(),'运行米长'])
        # self.doubleSpinBox_4.setValue(daoFile.loc[self.spinBox_5.value(),'运行米长'])
        # self.doubleSpinBox_5.setValue(daoFile.loc[self.spinBox_6.value(),'运行米长'])
        # self.doubleSpinBox_6.setValue(daoFile.loc[self.spinBox_7.value(),'运行米长'])
        # self.doubleSpinBox_7.setValue(daoFile.loc[self.spinBox_8.value(),'运行米长'])
        # self.doubleSpinBox_8.setValue(daoFile.loc[self.spinBox_9.value(),'运行米长'])
        # self.doubleSpinBox_9.setValue(daoFile.loc[self.spinBox_10.value(),'运行米长'])
        # self.doubleSpinBox_10.setValue(daoFile.loc[self.spinBox_11.value(),'运行米长'])
        # self.doubleSpinBox_11.setValue(daoFile.loc[self.spinBox_12.value(),'运行米长'])
        # self.doubleSpinBox_12.setVaule(daoFile.loc[self.spinBox_13.value(),'运行米长'])
    def alarmMeter(self):
        alarmlist = 'alarmNo.'
        for i in range(12):
...
class readPLCThread (threading.Thread):    #连续读取PLC数据
    ...
    def run(self):
        while True:
            ...
            api.badPoint = api.badPoint if (dec_to_float(result6[0],result6[1]) == None) else dec_to_float(result6[0],result6[1])
            api.badPoint = int(api.badPoint)
            ...
            # print('tongkuan',api.tongkuanUp,api.tongkuanDown)
            # print('badmeters',api.badMeters)
            # if api.badMeters != []:
            #     if not api.badMetersDone[api.badPoint]:
            #         if (abs(api.lengthAct-api.badMeters[api.badPoint]) <= 0.1) & (api.speedNow <= 0.05):
            #             api.badMetersDone[api.badPoint] = 1
            #             api.badPoint += 1
...
class mainStateMachine(object):         # 主状态机
    ...
    def on_enter_connectPLC(self):
        global client
        client = ModbusTcpClient('192.168.1.110')
        print('正在与PLC建立连接...')
