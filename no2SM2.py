...
def judgeNum():                       #数字判断
    ...
    if ((abs(msagint[5] - msagint[6]) >= 40) & (abs(msagint[5] - msagint[6]) <= 60)) & \
        ((abs(msagint[8] - msagint[9]) >= 8) & (abs(msagint[8] - msagint[9]) <= 40)) & \
        ((abs(msagint[10] - msagint[11]) >= 8) & (abs(msagint[10] - msagint[11]) <= 40)) & \
        ((abs(msagint[12] - msagint[13]) >= 8) & (abs(msagint[12] - msagint[13]) <= 40)) & \
        ((abs(msagint[14] - msagint[15]) >= 8) & (abs(msagint[14] - msagint[15]) <= 40)):
...
def calShoujuanPos(view,target):        #计算当前贴标或接带在收卷处的长度
    ...
    if view != -1:
        dist = api.fangjuanL + api.cam2wind1[0][target] - api.cam2view[0][0] + xleft
    elif view == -1:
...
def saveTempData():                   #保存临时数据
    ...
    api.cam2wind1 = [[8.37763,8.26452],[8.37763-0.0978,8.26452-0.0978]]  # [[上相机到上收卷之前，上相机到下收卷之前],[下相机到上收卷之前，下相机到下收卷之前]]
                            ...
                            'cameraPos':api.cameraPos, 'biaoliInverseFlag':api.biaoliInverseFlag, \
                                'offsetNY':api.offsetNY,'offsetPP':api.offsetPP,'offsetPPUp':api.offsetPPUp, \
                                    'offsetPPDown':api.offsetPPDown,'offsetX':api.offsetX,'lastPeiFangNum':api.lastPeiFangNum, \
                                        'knifeAlarmSet':api.knifeAlarmSet}])
...
class Api:                                 #仅用于设定全局变量
    def __init__(self):
        ...
        # self.cam2knife = [[7.65229+0.005,7.94342-0.065],[7.65229-0.0978+0.005,7.94342-0.0978-0.065]]  # [[上相机到切刀后上收卷停机,上相机到切刀后下收卷停机],[下相机到切刀后上收卷停机,下相机到切刀后下收卷停机]]
        ...
        self.offsetPPDown = 0       # NY面贴标补偿
        self.offsetPPUp = 0         # PP面贴标补偿
        ...
        self.shoujuanFlawStack = []     # 收卷贴标、接头记录栈[放卷长度，卷位，0贴标1接头]
...
class myMainWindow(QtWidgets.QMainWindow,Ui_MainWindow):    #主界面的函数
        ...
        self.spinBoxName = [self.spinBox,self.spinBox_3,self.spinBox_4,self.spinBox_5,self.spinBox_6,self.spinBox_7,self.spinBox_8,self.spinBox_9,\
            self.spinBox_10,self.spinBox_11,self.spinBox_12,self.spinBox_13,self.spinBox_14,self.spinBox_15,self.spinBox_16,self.spinBox_17,self.spinBox_18,self.spinBox_19,\
                self.spinBox_20,self.spinBox_22,self.spinBox_23,self.spinBox_24,self.spinBox_27,self.spinBox_28,self.spinBox_29,self.spinBox_25,self.spinBox_30,\
                    self.textEdit_2,self.textEdit_3,self.textEdit_10,self.textEdit_9,self.textEdit_11]
        self.displayData=[api.Meters_Stop,api.lengthSet,api.xintong_widthDown,api.fangjuanFSetDown,api.Mo_WidthDown,api.Mo_thickDown,api.tongkuanUp,api.circlesUp,\
            api.shoujuanFSetUp,api.zhuiduUp,api.shoujuanFSetDown,api.zhuiduDown,api.circlesDown,api.tongkuanDown,api.SpeedLmitHigh,api.Accelerate_time,api.Slow_time,api.Stop_time,\
                api.flawNums[api.badPoint],api.speedJog,api.didaoDingWei,int(api.Amount_LeftFlaw),api.badMeters[api.badPoint],api.alarmR,api.jiedaijinchi,api.deltaP,api.Meters_NextFlaw,\
                    api.offsetNY,api.offsetPP,api.offsetPPUp,api.offsetPPDown,api.offsetX]
    ...
    def mainTimer200(self):     # 主循环
        ...
        if self.mainState == 0:     #准备阶段
            ...
            self.laststate = self.mainState 
            if len(api.stopStack) > 0:
                if api.MXlist[1][11] & (not api.stopStack[-1][3]) & (abs(api.lengthAct-api.Meters_NextFlaw)<=0.1):  #瑕疵停机已经完成
                    ...
                    if api.dealtype != 0:
                        if api.dealtype == 2:       #接带，每个卷位的接带数量都要加1
                            ...
                            for ii in range(1,count1+1):
                                ...
                                # if ii in api.shoujuanFlawInfo.keys():
                                #     self.upOrDown = int(abs((api.firstUpOrDown - (1 - ii%2))))
                                #     api.shoujuanFlawStack.append([calShoujuanPos(api.lastStackPop[2], self.upOrDown),ii,1])
                                #     # api.shoujuanFlawInfo[ii][1].append(calShoujuanPos(api.lastStackPop[2], self.upOrDown))
                                # else:
                                #     api.shoujuanFlawInfo[ii] = [[],[calShoujuanPos(api.lastStackPop[2], self.upOrDown)]]
                        else:
                            api.shoujuanFlawStack.append([calShoujuanPos(api.lastStackPop[2], self.upOrDown),self.shoujuanPosNum,0])
                        ...
                        f6.close()
                            ...
                            #     api.shoujuanFlawInfo[self.shoujuanPosNum][0].append(calShoujuanPos(api.lastStackPop[2], self.upOrDown))
                            # else:
                            #     api.shoujuanFlawInfo[self.shoujuanPosNum] = [[calShoujuanPos(api.lastStackPop[2], self.upOrDown)],[]]
                        # print('total deals:', api.shoujuanFlawInfo)
    ...
    def readFlawFile(self):         # 读取缺陷文件
        ...
        root.withdraw()
        try:
            api.flawFilePath = filedialog.askdirectory(initialdir=flawFileFolderPath)
        except Exception:
            api.flawFilePath = updateFlawFile(flawFileFolderPath) + '\\ErrorData.csv'
    ...
    def addJietou(self):            #新增接头
        ...
        for ii in range(1,count1+1):
            self.upOrDown = int(abs((api.firstUpOrDown - (1 - ii%2))))
            api.shoujuanFlawStack.append([calShoujuanPos(-1,self.upOrDown),ii,1])
        api.shoujuanFlawStack.sort(key = lambda x:x[0], reverse = True)
...
class myKnifeAndPeifang(QtWidgets.QMainWindow,Ui_KnifeEdit):  #刀具操作界面
    def __init__(self):
        ...
        for i in range(12):
            ...
            # self.doubleSpinBoxNum[i].setValue(self.daoFile.loc[api.knifeUsing[i],'运行米长'])
...
class peiFang(QtWidgets.QWidget,Ui_Form1):  #配方编辑
    def __init__(self):
        ...
        self.pushButton.clicked.connect(self.savePeifang)
                ...
                api.Mo_thickDown,api.Meters_Stop,api.speedJog,api.xintongSizeUp,api.tongkuanUp,\
                    api.circlesUp,api.shoujuanFSetUp,api.zhuiduUp,api.xintongSizeDown,api.tongkuanDown,\
                        api.circlesDown,api.shoujuanFSetDown,api.zhuiduDown,api.Accelerate_time,api.Slow_time,\
                            api.Stop_time]
        self.peifangSpinBox = [self.textEdit_23,self.spinBox_2,self.spinBox_3,self.spinBox_4,self.spinBox_7,self.spinBox_5,self.spinBox_6,self.spinBox_8,\
    ...
    def peiFangNow(self):
        ...
        for i in range(1,len(self.textEditName)):
                self.textEditName[i].setPlainText(str(self.NowValue[i]))
        # print(list(readCsv().loc[5]))
        self.textEdit_22.setPlainText(list(readCsv().loc[5])[0])
    ...
    def modifyPeifang(self):
        ...
        peifangSpinBox = [self.textEdit_23,self.spinBox_2,self.spinBox_3,self.spinBox_4,self.spinBox_7,self.spinBox_5,self.spinBox_6,self.spinBox_8,\
            self.spinBox_9,self.spinBox_10,self.spinBox_11,self.spinBox_12,self.spinBox_13,self.spinBox_14,self.spinBox_15,self.spinBox_16,self.spinBox_17,\
                self.spinBox_18,self.spinBox_19,self.spinBox_20,self.spinBox_21,self.spinBox_22]
        NumPeiFang = peiFangWin.spinBox.value()
