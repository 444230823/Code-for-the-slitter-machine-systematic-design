...
class myMainWindow(QtWidgets.QMainWindow,Ui_MainWindow,QObject):    #主界面的函数
    def __init__(self):
        ...
        self.dataShow()
        self.sendstate = pyqtSignal(object)
        # global last_qizhang
        ...
        # flaw_rise = 0
        # 机器状态线程
        self.threadmachinestate = machinestateThread(4,'Thread4',0)
        threadpool.start(self.threadmachinestate)
    ...
    def pressedFcn(self,mw,mx,stay):     # 鼠标按下的函数
        ...
        if mw!=999:
            ...
            if api.uiEA:
                ...
                if mw == 9:
                    if buttonList[mx] == 1:#瑕疵选择打开
                        self.sendstate.emit(1)
                        # self.threadmachinestate.func31()
                    else:
                        self.threadmachinestate.state = 2
        ...
        if api.uiEA:
            ...
            if (self.sender() == self.textEdit_2) | (self.sender() == self.textEdit_3):
                if (lastoffsetNY != api.offsetNY) | (lastoffsetPP != api.offsetPP):
                    # self.threadmachinestate.func33()
                    self.sendstate.emit(3)
                # saveTempData()
            if (a == -1):
    ...
    def readFlawFile(self):         # 读取缺陷文件
        ...
        # threadReadPLC.state = 32
        self.sendstate.emit(2)
    ...
    def resetSoftware(self):    # 程序复位
        ...
        makeDecision(api.flawFilePath + '/ErrorData.csv')
        time.sleep(0.5)
        self.threadmachinestate.state = 1
...
class readPLCThread (QRunnable):    #连续读取PLC数据
    def __init__(self, threadID, name, stopflag):
        ...
        global client
        client = ModbusTcpClient('10.30.76.30')
...
class machinestateThread(QRunnable,QObject):
    #初始化
    def __init__(self, threadID, name, stopflag):
        super(machinestateThread, self).__init__()
        self.threadID = threadID
        self.name = name
    ...
    def run(self):
        ...
        match self.state:
            ...
            case 2:     # 准备阶段
                ...
                # 瑕疵停机打开
                if api.MXlist[5][2]:
                    if self.firstStart:# 第一次进
                        self.func33()
                        self.func31()
                        self.firstStart = False
                    else:
                        if self.flaw_rise or self.qizhang_rise: #有个气涨
                            if self.lastDeal == 2:
                                self.func34()
                            else:
                                self.func31()
            ...
            case 3:     #判断当前目标停机是否完成
                ...
                if len(api.stopStack) > 0:
                    if (api.MXlist[1][11] & (not api.stopStack[-1][3])) | (self.handyPopFlag == 1):  #瑕疵停机已经完成，或手动打开瑕疵停机弹窗
                        if ((api.stopStack[-1][2] <= 1) & (abs(api.lengthAct-api.Meters_NextFlaw)<=0.5) & (abs(api.actSpeed)<=1)) | \
                            ((api.stopStack[-1][2] > 1) & (abs(api.lengthAct-api.Meters_NextFlaw)<=0.2) & (abs(api.actSpeed)<=1)) | \
                            ...
                            if api.stopStack[-1][2] > 1:
                                ...
                                if api.stopStack[-1][2] != 6:
                                    try:
                                        client.write_registers (values=inverse(findGongWeiNum(api.badMetersX[api.findPoint[api.stopStack[-1][0]]])), unit=2, address=564, data_format='!f')
                                    except:
                                        pass
                                request1 = client.read_holding_registers(9,1).registers
                                buttonList = dec_to_binlist(request1[0],16)
            ...
            case 5:     #根据工人在弹窗中的操作，确定对当前停机的处理方式
                api.systemState = '系统正常'
                if self.laststate != self.state:
                    print('mainState:',self.state)
                self.laststate = self.state
                ...
                if api.lengthAct < api.stopStack[-1][1]:
                    ...
                    for j in range(len(api.stopStack)-1):
                        if (api.lengthAct < api.stopStack[j][1]) & (api.lengthAct >= api.stopStack[j+1][1]):
                            ...
                            break
                if api.stopStack[-1][0] != 0:
                    api.badPoint = api.findPoint[api.stopStack[-1][0]]#缺陷索引
                if api.ordertype == 1:
    ...
    def func32(self):#空栈时候，或者新读取了缺陷文件之后，重新把当前位置后的所有缺陷进栈
        api.systemState = '系统正常'
        if self.laststate != self.state:
            print('mainState:',self.state)
        self.laststate = self.state 
...
if __name__=='__main__':
    ...
    mainWin.sendstate.connect(mainWin.threadmachinestate.getflag)
