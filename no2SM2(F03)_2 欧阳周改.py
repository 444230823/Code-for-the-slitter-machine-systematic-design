...
from pymodbus.client import ModbusTcpClient  # 修改了 from pymodbus.client.sync import ModbusTcpClient
...
# readPath = os.getcwd()
# readPath = os.path.join(CurrentPath, 'csvPath') 
readPath = 'D:/PythonWorks'
...
flawPath = os.path.expanduser('~')
flawPath = os.path.join(flawPath,'Desktop')
# flawPath = 'C:/Users/GADE2/Desktop'
...
def judgeNum():                       #数字判断
    ...
    if ((abs(msagint[5] - msagint[6]) >= 35) & (abs(msagint[5] - msagint[6]) <= 60)) & \
...
class photoThread (QRunnable):      #采集图像类
    def __init__(self, threadID, name, stopflag):
        ...
        self.target_host = "10.30.76.27"
        self.target_port = 8500
        global client2
        #建立socket对象
            ...
            match self.state:
                ...
                case 4:     # 字符识别
                        ...
                        for a in reader:
                            for i in range(23):
                                if (i == 0) | ((i >= 5)&(i <= 15)) | ((i >= 17)&(i <= 20)):
                                    a[i] = float(a[i])
                        f3.close()
                        if ((float(msagint[0]) > 0)&(msagint[1] != ' ')&(msagint[2] != ' ')&(msagint[3] != ' ')&(msagint[4] != ' ')):  
                    ...
                    if total >= 100000:
                        ...
                        csv.writer(f).writerow(titles)
                        total = 1
                    if self.state != 1:     # 防止刷新太快无法获取到外部程序对状态的改变
                        self.state = 2
                    # client2.close()
...
class Api:                                 #仅用于设定全局变量
    ...
    def initialUIShow(self):
        ...
        self.actSpeed = 0           # AXIS3实际转速

        self.PLCConState = 0        # PLC连接状态，0连接失败，1连接成功
        self.CamConState = 0        # 相机连接状态，0连接失败，1连接成功
        self.systemState = ''       # 系统当前运行状态显示
        self.shutDownFlag = 0       # 关闭线程标志
        self.popWinFlag = 0         # 瑕疵处理弹窗标志
        self.closePopWinFlag = 0
        ...
        self.firstUpOrDown = 0  # 1工位在0上收卷还是1下收卷,在文件最后一行
        ...
        if os.path.exists(lastFlawPath):
            ...
            self.flawFilePath = updateFlawFile(flawFileFolderPath)
        # makeDecision(self.flawFilePath + '/ErrorData.csv')
        ...
        if os.path.exists(readPath + '\\stopStack.csv'):    # 停靠任务栈
            f7 = open(readPath + '\\stopStack.csv', 'r', newline='')
            reader = csv.reader(f7)
            for i in reader:
                self.stopStack.append([int(i[0]), float(i[1]), int(i[2]), int(i[3])])
            print('in_------',self.stopStack)
...
class myMainWindow(QtWidgets.QMainWindow,Ui_MainWindow):    #主界面的函数
    def __init__(self):
        ...
        self.pushButton_29.clicked.connect(self.resetSoftware)      # 复位程序
        self.pushButton_32.clicked.connect(self.quitSoftware)       # 退出程序
        #按键按下时触发函数
        ...
        self.dataShow()
        self.cpi = CPI(1, 'Thread1', 0)
        threadpool.start(self.cpi)
        self.timer200 = QtCore.QTimer()
        self.timer200.timeout.connect(self.mainTimer100)
        self.timer200.start(100)

    def mainTimer100(self):     # 主循环
        self.continueShow()
        if api.fangjuanL != 0:
        ...
        if api.popWinFlag:
            global flawDealWin
            api.popWinFlag = 0
            flawDealWin.initialPopUI()
            flawDealWin.show()
            print(api.stopStack[-1],' poped once')
        if api.closePopWinFlag:
            # global flawDealWin
            api.closePopWinFlag = 0
            try:
                flawDealWin.close()
            except Exception as e:
                print(e)
        # if mainWin.cpi.state == 4:
        #     if len(api.stopStack) > 0:
        #         if (api.MXlist[1][11] & (not api.stopStack[-1][3])) | (threadReadPLC.handyPopFlag == 1):  #瑕疵停机已经完成，或手动打开瑕疵停机弹窗
        ...
        #                 (threadReadPLC.handyPopFlag == 1):   #观察区停机误差0.5，处理区停机误差0.2
        #                 threadReadPLC.handyPopFlag = 0
    ...
    def continueShow(self):
        ...
        self.textEdit_5.setPlainText(api.systemState)
        ...
        if api.PLCConState:     # PLC连接状态显示
            self.pushButton_30.setStyleSheet("background-color: #00ff00")
        else:
            self.pushButton_30.setStyleSheet("background-color: #ff0000")
        if api.CamConState:     # 相机连接状态显示
            self.pushButton_31.setStyleSheet("background-color: #00ff00")
        else:
            self.pushButton_31.setStyleSheet("background-color: #ff0000")
    ...
    def qtInput(self,a):
        if api.uiEA:
            ...
            lastoffsetNY            = api.offsetNY    
            lastoffsetPP            = api.offsetPP    
            lastoffsetPPUp          = api.offsetPPUp  
            lastoffsetPPDown        = api.offsetPPDown
            lastoffsetX             = api.offsetX     
            lastoffsetNYUp          = api.offsetNYUp  
            lastoffsetNYDown        = api.offsetNYDown
            ...
            if (self.sender() == self.textEdit_2) | (self.sender() == self.textEdit_3):
                if (lastoffsetNY != api.offsetNY) | (lastoffsetPP != api.offsetPP):
                    self.cpi.state = 33
                # saveTempData()
            if (a == -1):
                if (lastoffsetPPUp != api.offsetPPUp) | (lastoffsetPPDown != api.offsetPPDown) | \
                    (lastoffsetX != api.offsetX) | (lastoffsetNYUp != api.offsetNYUp) | \
                    (lastoffsetNYDown != api.offsetNYDown):
    ...
    def handyPop(self):
        threadReadPLC.handyPopFlag = 1   # 启动手动弹窗
    ...
    def resetSoftware(self):    # 程序复位
        ...
        time.sleep(0.5)
        self.cpi.state = 1
        threadCamera.state = 1

    def quitSoftware(self):     # 程序退出
        ans = QMessageBox.question(self,'程序退出确认','确认关闭程序？',QMessageBox.Yes|QMessageBox.No)
    ...
    def saveAllFiles(self):     # 保存全部文件
        ...
        # if api.lastRightNum != []:
        #     f3 = open(readPath + '\\camTemp.csv','w',newline='')
        #     csv.writer(f3).writerow(api.lastRightNum)
        #     f3.close()
        # 刀具文件保存
        try:
        ...
        f6.close()
        # 停靠任务栈保存
        f7 = open(readPath + '\\stopStack.csv', 'w', newline='')
        csv.writer(f7).writerows(api.stopStack)
        f7.close()
...
class myKnifeAndPeifang(QtWidgets.QMainWindow,Ui_KnifeEdit):  #刀具操作界面
    def __init__(self):
        ...
        self.pushButton_5.clicked.connect(lambda:self.meterClearButton(self.spinBox_2.value()))
        self.pushButton_6.clicked.connect(lambda:self.meterClearButton(self.spinBox_3.value()))
        self.pushButton_7.clicked.connect(lambda:self.meterClearButton(self.spinBox_4.value()))
        self.pushButton_8.clicked.connect(lambda:self.meterClearButton(self.spinBox_5.value()))
        self.pushButton_9.clicked.connect(lambda:self.meterClearButton(self.spinBox_6.value()))
        self.pushButton_10.clicked.connect(lambda:self.meterClearButton(self.spinBox_7.value()))
        self.pushButton_11.clicked.connect(lambda:self.meterClearButton(self.spinBox_8.value()))
        self.pushButton_12.clicked.connect(lambda:self.meterClearButton(self.spinBox_9.value()))
        self.pushButton_13.clicked.connect(lambda:self.meterClearButton(self.spinBox_10.value()))
        self.pushButton_14.clicked.connect(lambda:self.meterClearButton(self.spinBox_11.value()))
        self.pushButton_15.clicked.connect(lambda:self.meterClearButton(self.spinBox_12.value()))
        self.pushButton_16.clicked.connect(lambda:self.meterClearButton(self.spinBox_13.value()))
    ...
    def meterClearButton(self,daoNum):
...
class peiFang(QtWidgets.QWidget,Ui_Form1):  #配方编辑
    ...
    def peiFangNow(self):       #显示当前配方
        ...
        # print(list(self.readCsv().loc[5]))
        self.textEdit_22.setPlainText(str(self.readCsv().loc[api.lastPeiFangNum,'配方名']))
    ...
    def peifangApp(self):  #显示选择的配方
        ...
        NumRead = list(self.readCsv().loc[NumPeiFang])
        for k in range(22):
    ...
    def selectPeiFang(self):
        ...
        if NumPeiFang==0:#机器上的配方
            ...
            for k in range(22):
                if k == 0:
                    # self.peifangSpinBox[k].setPlainText(list(self.readCsv().loc[5])[0])
                    self.peifangSpinBox[k].setPlainText(str(self.readCsv().loc[NumPeiFang,'配方名']))
                else:
...
class flawDealPopup(QtWidgets.QMainWindow,Ui_DefectProcess):  #缺陷处理弹窗
    ...
    def initialPopUI(self):
        print('pop win initializing')
        self.setupUi(self)
        self.pushButton.clicked.connect(self.openFlawStop)
        self.pushButton_2.clicked.connect(self.show_pic)
        self.pushButton_3.clicked.connect(self.openFlawFile)
        ...
        if self.flawShow[2] > 1:        # 如果是两个观察区后面的停机位置，则不需要选择处理方式
...
class shoujuanGongwei(QtWidgets.QMainWindow,Ui_ShouJuan):  #收卷工位设置
    def __init__(self):
        ...
        print('分切表面 {}'.format(api.cutPPorNY))
        if api.cutPPorNY == 0:
            self.radioButton_5.setChecked(True)
            self.radioButton_6.setChecked(False)
        ...
        for i in range(len(self.dsbNames)):
            self.dsbNames[i].setValue(api.shoujuanWidths[i])
        if api.firstUpOrDown == 0:
            self.radioButton.setChecked(True)
            self.radioButton_2.setChecked(False)
        elif api.firstUpOrDown:
            self.radioButton.setChecked(False)
            self.radioButton_2.setChecked(True)
...
class readPLCThread (QRunnable):    #连续读取PLC数据
    def __init__(self, threadID, name, stopflag):
        super(readPLCThread,self).__init__()
        self.threadID = threadID
        self.name = name
        self.stop = stopflag
        self.state=0
        ...
        self.cutStart = 0       # 接带平台中心开始裁剪的位置
        self.cutEnd = 0         # 接带平台中心结束裁剪的位置
        self.lastDeal = 0       # 上一个瑕疵的处理方式
        self.laststate = 0      # 状态机的上一个状态
        makeDecision(api.flawFilePath + '/ErrorData.csv')
    ...
    def run(self):
        while True:
            ...
            match state:
                case 0:
                        print('正在与PLC建立连接...')
                        api.systemState = '正在连接PLC与相机...'
                        api.PLCConState = client.connect()
                        if api.PLCConState:
                            self.state = 1
                        else:
                            ...
                            self.state = 0
                            if api.CamConState:
                                api.systemState = '与PLC连接失败！正在尝试重连'
                            else:
                                api.systemState = '与PLC连接失败！与相机连接失败！正在尝试重连'
                case 1:
                        self.PLCtoApi()
    ...
    def PLCtoApi(self):
            ...
            try:
                self.request0 = client.read_holding_registers(0,10).registers
                self.result1 = client.read_holding_registers(10,54).registers
                self.result2 = client.read_holding_registers(98,18).registers
                self.result3 = client.read_holding_registers(416,8).registers
                self.result4 = client.read_holding_registers(444,8).registers
                self.result5 = client.read_holding_registers(132,10).registers
                self.result6 = client.read_holding_registers(188,10).registers
                self.result7 = client.read_holding_registers(470,2).registers
                self.result8 = client.read_holding_registers(76,2).registers
                api.PLCConState = 1
            except Exception as e:
                api.PLCConState = 0
                print(e)
            ...
            # MX寄存器
            api.MXlist[0] = dec_to_binlist(self.request0[0],16)
            api.MXlist[1] = dec_to_binlist(self.request0[1],16)
            api.MXlist[2] = dec_to_binlist(self.request0[2],16)
            api.MXlist[3] = dec_to_binlist(self.request0[3],16)
            api.MXlist[4] = dec_to_binlist(self.request0[4],16)
            api.MXlist[5] = dec_to_binlist(self.request0[9],16)
            # real型变量
            api.speedNow = api.speedNow if (dec_to_float(self.result1[0],self.result1[1]) == None) else dec_to_float(self.result1[0],self.result1[1])
            api.lengthAct = api.lengthAct if (dec_to_float(self.result1[2],self.result1[3]) == None) else dec_to_float(self.result1[2],self.result1[3])
            api.LengthSum = api.LengthSum if (dec_to_float(self.result1[4],self.result1[5]) == None) else dec_to_float(self.result1[4],self.result1[5])
            api.houshoujuanR = api.houshoujuanR if (dec_to_float(self.result1[6],self.result1[7]) == None) else dec_to_float(self.result1[6],self.result1[7])
            api.QianshoujuanR = api.QianshoujuanR if (dec_to_float(self.result1[8],self.result1[9]) == None) else dec_to_float(self.result1[8],self.result1[9])
            api.fangjuanActFUp = api.fangjuanActFUp if (dec_to_float(self.result1[10],self.result1[11]) == None) else dec_to_float(self.result1[10],self.result1[11])
            api.fangjuanRUp = api.fangjuanRUp if (dec_to_float(self.result1[12],self.result1[13]) == None) else dec_to_float(self.result1[12],self.result1[13])
            api.fangjuanActFDown = api.fangjuanActFDown if (dec_to_float(self.result1[14],self.result1[15]) == None) else dec_to_float(self.result1[14],self.result1[15])
            api.fangjuanRDown = api.fangjuanRDown if (dec_to_float(self.result1[16],self.result1[17]) == None) else dec_to_float(self.result1[16],self.result1[17])
            api.SpeedLmitHigh = api.SpeedLmitHigh if (dec_to_float(self.result1[18],self.result1[19]) == None) else dec_to_float(self.result1[18],self.result1[19])
            api.SpeedLmitLow = api.SpeedLmitLow if (dec_to_float(self.result1[20],self.result1[21]) == None) else dec_to_float(self.result1[20],self.result1[21])
            api.lengthSet = api.lengthSet if (dec_to_float(self.result1[22],self.result1[23]) == None) else dec_to_float(self.result1[22],self.result1[23])
            api.xintong_widthUp = api.xintong_widthUp if (dec_to_float(self.result1[24],self.result1[25]) == None) else dec_to_float(self.result1[24],self.result1[25])
            api.fangjuanFSetUp = api.fangjuanFSetUp if (dec_to_float(self.result1[26],self.result1[27]) == None) else dec_to_float(self.result1[26],self.result1[27])
            api.Mo_WidthUp = api.Mo_WidthUp if (dec_to_float(self.result1[28],self.result1[29]) == None) else dec_to_float(self.result1[28],self.result1[29])
            api.Mo_thickUp = api.Mo_thickUp if (dec_to_float(self.result1[30],self.result1[31]) == None) else dec_to_float(self.result1[30],self.result1[31])
            api.xintongTypeUp = api.xintongTypeUp if (dec_to_float(self.result1[32],self.result1[33]) == None) else dec_to_float(self.result1[32],self.result1[33])
            api.moTypeUp = api.moTypeUp if (dec_to_float(self.result1[34],self.result1[35]) == None) else dec_to_float(self.result1[34],self.result1[35])
            api.xintong_widthDown = api.xintong_widthDown if (dec_to_float(self.result1[36],self.result1[37]) == None) else dec_to_float(self.result1[36],self.result1[37])
            api.fangjuanFSetDown = api.fangjuanFSetDown if(dec_to_float(self.result1[38],self.result1[39]) == None) else dec_to_float(self.result1[38],self.result1[39])
            api.Mo_WidthDown = api.Mo_WidthDown if(dec_to_float(self.result1[40],self.result1[41]) == None) else dec_to_float(self.result1[40],self.result1[41])
            api.Mo_thickDown = api.Mo_thickDown if(dec_to_float(self.result1[42],self.result1[43]) == None) else dec_to_float(self.result1[42],self.result1[43])
            api.xintongTypeDown = api.xintongTypeDown if(dec_to_float(self.result1[44],self.result1[45]) == None) else dec_to_float(self.result1[44],self.result1[45])
            api.moTypeDown = api.moTypeDown if(dec_to_float(self.result1[46],self.result1[47]) == None) else dec_to_float(self.result1[46],self.result1[47])
            api.Accelerate_time = api.Accelerate_time if (dec_to_float(self.result1[48],self.result1[49]) == None) else dec_to_float(self.result1[48],self.result1[49])
            api.Slow_time = api.Slow_time if (dec_to_float(self.result1[50],self.result1[51]) == None) else dec_to_float(self.result1[50],self.result1[51])
            api.Stop_time = api.Stop_time if (dec_to_float(self.result1[52],self.result1[53]) == None) else dec_to_float(self.result1[52],self.result1[53])

            api.xintongSizeUp = api.xintongSizeUp if (dec_to_float(self.result2[0],self.result2[1]) == None) else dec_to_float(self.result2[0],self.result2[1])
            api.xintongSizeDown = api.xintongSizeDown if (dec_to_float(self.result2[2],self.result2[3]) == None) else dec_to_float(self.result2[2],self.result2[3])
            api.didaoDingWei = api.didaoDingWei if (dec_to_float(self.result2[4],self.result2[5]) == None) else dec_to_float(self.result2[4],self.result2[5])
            api.Meters_Stop = api.Meters_Stop if (dec_to_float(self.result2[6],self.result2[7]) == None) else dec_to_float(self.result2[6],self.result2[7])
            api.yagunBackUp = api.yagunBackUp if (dec_to_float(self.result2[8],self.result2[9]) == None) else dec_to_float(self.result2[8],self.result2[9])
            api.yagunBackDown = api.yagunBackDown if (dec_to_float(self.result2[10],self.result2[11]) == None) else dec_to_float(self.result2[10],self.result2[11])
            api.speedJog = api.speedJog if (dec_to_float(self.result2[12],self.result2[13]) == None) else dec_to_float(self.result2[12],self.result2[13])
            api.offsetNY = api.offsetNY if (dec_to_float(self.result2[14],self.result2[15]) == None) else dec_to_float(self.result2[14],self.result2[15])
            api.offsetPP = api.offsetPP if (dec_to_float(self.result2[16],self.result2[17]) == None) else dec_to_float(self.result2[16],self.result2[17])

            api.fangjuanSpeedRatio = api.fangjuanSpeedRatio if (dec_to_float(self.result5[0],self.result5[1]) == None) else dec_to_float(self.result5[0],self.result5[1])
            api.alarmR = api.alarmR if (dec_to_float(self.result5[4],self.result5[5]) == None) else dec_to_float(self.result5[4],self.result5[5])
            api.tongkuanDown = api.tongkuanDown if (dec_to_float(self.result5[6],self.result5[7]) == None) else dec_to_float(self.result5[6],self.result5[7])
            api.tongkuanUp = api.tongkuanUp if (dec_to_float(self.result5[8],self.result5[9]) == None) else dec_to_float(self.result5[8],self.result5[9])

            api.shoujuanFSetUp = api.shoujuanFSetUp if (dec_to_float(self.result3[0],self.result3[1]) == None) else dec_to_float(self.result3[0],self.result3[1])
            api.shoujuanFSetDown = api.shoujuanFSetDown if (dec_to_float(self.result3[2],self.result3[3]) == None) else dec_to_float(self.result3[2],self.result3[3])
            api.zhuiduUp = api.zhuiduUp if (dec_to_float(self.result3[4],self.result1[5]) == None) else dec_to_float(self.result3[4],self.result3[5])
            api.zhuiduDown = api.zhuiduDown if (dec_to_float(self.result3[6],self.result3[7]) == None) else dec_to_float(self.result3[6],self.result3[7])

            api.targetUp = api.targetUp if (dec_to_float(self.result4[0],self.result4[1]) == None) else dec_to_float(self.result4[0],self.result4[1])
            api.targetDown = api.targetDown if (dec_to_float(self.result4[2],self.result4[3]) == None) else dec_to_float(self.result4[2],self.result4[3])
            api.circlesUp = api.circlesUp if (dec_to_float(self.result4[4],self.result1[5]) == None) else dec_to_float(self.result4[4],self.result4[5])
            api.circlesDown = api.circlesDown if (dec_to_float(self.result4[6],self.result4[7]) == None) else dec_to_float(self.result4[6],self.result4[7])

            api.flawPoint = api.flawPoint if (dec_to_float(self.result6[0],self.result6[1]) == None) else dec_to_float(self.result6[0],self.result6[1])
            api.flawPoint = int(api.flawPoint)
            api.Amount_LeftFlaw = api.Amount_LeftFlaw if (dec_to_float(self.result6[2],self.result6[3]) == None) else dec_to_float(self.result6[2],self.result6[3])
            api.Meters_NextFlaw = api.Meters_NextFlaw if (dec_to_float(self.result6[4],self.result6[5]) == None) else dec_to_float(self.result6[4],self.result6[5])
            api.jiedaijinchi = api.jiedaijinchi if (dec_to_float(self.result6[8],self.result6[9]) == None) else dec_to_float(self.result6[8],self.result6[9])

            api.fangjuanL = api.fangjuanL if (dec_to_float(self.result7[0],self.result7[1]) == None) else dec_to_float(self.result7[0],self.result7[1])
            api.actSpeed = api.actSpeed if (dec_to_float(self.result8[0],self.result8[1]) == None) else dec_to_float(self.result8[0],self.result8[1])
            
            # 如果进行了放卷清零，收卷报表栈内所有目标减去之前的放卷长度
            if (api.fangjuanL == 0) & (self.lastFangjuan > 0):
                if len(api.shoujuanFlawStack) > 0:
                    for i in len(api.shoujuanFlawStack):
                        api.shoujuanFlawStack[i][0] -= self.lastFangjuan
                        
            # 根据收卷报表栈与放卷长度计算瑕疵点是否已收入产品卷
            if len(api.shoujuanFlawStack) > 0:
    ...
    def run(self):
        while True:
            match self.state:
                case 1:  # 初始化
                    # api.initialUIShow()
                    api.systemState = '系统正常'
                    self.handyPopFlag = 0  # 手动瑕疵停机弹窗标志复位
                    self.state = 2
                    self.now_qizhang = api.QXlist[15]
                    self.last_qizhang = self.now_qizhang
                    self.now_flawStop = api.MXlist[5][2]
                    self.last_flawStop = self.now_flawStop
                    self.qizhang_rise = 0
                    self.flaw_rise = 0
                ...
                case 2:  # 准备阶段
                    ...
                    self.now_qizhang = api.QXlist[15]
                    self.now_flawStop = api.MXlist[5][2]
                    self.qizhang_rise = 1 if ((self.last_qizhang == 0) & (self.now_qizhang == 1)) else 0
                    self.flaw_rise = 1 if ((self.last_flawStop == 0) & (self.now_flawStop == 1)) else 0
                    if api.MXlist[5][2]:#瑕疵停机选择按钮
                        if self.firstStart:
                            self.state = 31
                    ...
                    self.last_qizhang = self.now_qizhang
                    self.last_flawStop = self.now_flawStop
                case 3:  # 检查是否刚打开气胀或瑕疵停机
                    api.systemState = '系统正常'
                    if self.qizhang_rise or self.flaw_rise:#进入初始化
                        if self.lastDeal == 2:
                            self.state = 34
                        else:
                            self.state = 31
                        ...
                        if api.lengthAct >= api.stopStack[0][1]:  # 如果后面没有缺陷了，直接跑完剩下的膜料
                            ...
                            if api.lengthAct < api.stopStack[-1][1]:
                                ...
                                for j in range(len(api.stopStack) - 1):
                                    if (api.lengthAct < api.stopStack[j][1]) & (
                                            api.lengthAct >= api.stopStack[j + 1][1]):
                            ...
                            if api.Meters_NextFlaw > api.lengthAct:
                                client.write_registers(
                                    values=inverse(api.stopStack[-1][0]) + inverse(api.Amount_LeftFlaw) + inverse(
                                        api.Meters_NextFlaw), unit=2, address=188, data_format='!f')
                                client.write_registers(values=inverse(api.zhengfan[api.badPoint]), unit=2, address=300,
                                                       data_format='!f')
                ...
                case 32:  # 空栈时候，或者新读取了缺陷文件之后，重新把当前位置后的所有缺陷进栈
                    api.systemState = '系统正常'
                    if self.laststate != self.state:
                        print('mainState:', self.state)
                    self.laststate = self.state
                ...
                case 34:  # 裁剪后，删除裁掉的缺陷
                    ...
                    self.state = 31
                case 4:  # 判断当前目标停机是否完成
                    api.systemState = '等待瑕疵停机完成'
                    if self.laststate != self.state:
                        print('mainState:', self.state)
                    self.laststate = self.state
                    if len(api.stopStack) > 0:
                        if (api.MXlist[1][11] & (not api.stopStack[-1][3])) | (
                                self.handyPopFlag == 1):  # 瑕疵停机已经完成，或手动打开瑕疵停机弹窗
                            if ((api.stopStack[-1][2] <= 1) & (abs(api.lengthAct - api.Meters_NextFlaw) <= 0.5) & (
                                    abs(api.actSpeed) <= 1)) | \
                                    ((api.stopStack[-1][2] > 1) & (abs(api.lengthAct - api.Meters_NextFlaw) <= 0.2) & (
                                            abs(api.actSpeed) <= 1)) | \
                                ...
                                if api.stopStack[-1][2] > 1:
                                    client.write_registers(values=inverse(api.stopStack[-1][2]), unit=2, address=560,
                                                           data_format='!f')  # DealType_r_g 写入停机区代号
                                    if api.stopStack[-1][2] != 6:
                                        try:
                                            client.write_registers(values=inverse(
                                                findGongWeiNum(api.badMetersX[api.findPoint[api.stopStack[-1][0]]])),
                                                                   unit=2, address=564, data_format='!f')
                                        except:
                                            pass
                                    request1 = client.read_holding_registers(9, 1).registers
                                    buttonList = dec_to_binlist(request1[0], 16)
                ...
                case 5:  # 等待工人对弹窗的操作完成
                    if api.stopStack[-1][2] == 6:
                        api.systemState = '预设米长到达！'
                    else:
                        api.systemState = '等待弹窗处理完成'
                    if self.laststate != self.state:
                        print('mainState:', self.state)
                    self.laststate = self.state
                    request1 = client.read_holding_registers(9, 1).registers
                    ...
                    if api.dealDoneFlag and (not buttonList[4]):  # 在上位机点击了完成
                        ...
                        pass
                case 6:  # 根据工人在弹窗中的操作，确定对当前停机的处理方式
                    api.systemState = '系统正常'
                    if self.laststate != self.state:
                        print('mainState:', self.state)
                    self.laststate = self.state
                    try:
                        request0 = client.read_holding_registers(1, 1).registers
                        list1 = dec_to_binlist(request0[0], 16)
                        list1[11] = 0
                        command = binlist_to_int(list1)
                        client.write_register(value=command, unit=2, address=1)
                    except Exception as e:
                        print(e)
                    if api.stopStack[-1][3]:
                        ...
                        if len(api.stopStack) >= 0:  # 还有剩余停机点
                            ...
                            self.upOrDown = int(abs((api.firstUpOrDown - (
                                        1 - self.shoujuanPosNum % 2))))  # 根据x坐标判断收卷在第几工位，进而判断在上收卷还是下收卷
                            ...
                            if api.dealtype != 0:
                                if api.lastStackPop[2] <= 1:  # 观察区停机
                                    if api.dealtype != 2:  # 接带，每个卷位的接带数量都要加1
                                        api.shoujuanFlawStack.append(
                                            [calShoujuanPos(api.lastStackPop[2], self.upOrDown), self.shoujuanPosNum, 0,
                                             api.lastStackPop[0]])
                                    api.shoujuanFlawStack.sort(key=lambda x: x[0], reverse=True)
                                    # f6 = open(readPath + '\\shoujuanStack.csv','w',newline='')
                                    # csv.writer(f6).writerows(api.shoujuanFlawStack)
...
if __name__=='__main__':
    ...
    api = Api()
    time.sleep(0.5)
    threadReadPLC = readPLCThread(2,'Thread2',0)
    threadpool.start(threadReadPLC)
    time.sleep(1)

    threadCamera = photoThread(3,'Thread3',0)
    threadpool.start(threadCamera)
 
    global lineWin, knifeWin, peiFangWin, mainWin, flawDealWin
    try:
        os.open(api.flawFilePath + '/ErrorData.csv')
    except Exception as e:
        print(e)
    makeDecision(api.flawFilePath + '/ErrorData.csv')

    app = QtWidgets.QApplication(sys.argv)
    flawDealWin = flawDealPopup()
